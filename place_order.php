...
// var_dump("686.20" >= "891.00");
// var_dump((int)"686.20");
// var_dump((int)"891.00");
// var_dump((int)$total_user_money->money);
// var_dump((int)$products[count($products) - 1]["total_order_price"]);
// var_dump($products[count($products) - 1]["total_order_price"]);
// var_dump((int)$total_user_money->money >= (int)$products[count($products) - 1]["total_order_price"]);
	if ((int)$total_user_money->money >= (int)$products[count($products) - 1]["total_order_price"])
	{
		...
		$total_order_price = $products[count($products) - 1]["total_order_price"];
		$new_user_money = ($total_user_money->money - $total_order_price);

// var_dump(number_format($new_user_money, 2, '.', ''));
// var_dump($user->id);
		$query = $db_con->prepare("UPDATE user SET money = ? WHERE id = ?");
		$statement ->bind_param("di", number_format($new_user_money, 2, '.', ''), $user->id);
